...
wall_normal = {math.cos(theta + 90*math.pi/180), math.sin(theta + 90*math.pi/180)}
--karman constant
k = 0.41
function u_x_initial(x,y,z)
  dist = (wall_normal[1]*x + wall_normal[2]*y)
  inclined_height = (length*math.cos(theta)*wall_normal[1] + (length*(math.sin(theta))+height)*wall_normal[2])
  --print(x,y,dist)
  if dist<=inclined_height*0.5 then
    y_plus = vel_fric_phy*dist/nu_phy
  else
    y_plus = vel_fric_phy*(inclined_height-dist)/nu_phy
  end
  ...
  turb_channel_force_dynamic = {
    ...
    shape_umean = {
      ...
      object = {
        ...
        vec = {
          {length*(math.cos(theta)), length*(math.sin(theta)), 0.0},
          {0.0, height, 0.0},
          {0, 0, width}
        }
      },
    ...
    flow_direction = 'x',
    unit_stream_wise = {math.cos(theta), math.sin(theta), 0.0},
    unit_wall_normal = {math.cos(theta + 90*math.pi/180), math.sin(theta + 90*math.pi/180), 0.0}
  }
}
--! [Source term]
...
variable = {
  ...
  { --has to be added to the attributes right behind kind and label
...
tracking = {
  {
    ...
    time_control = {min= 0, max = tmax_phy, interval = T_c/10},
    --time_control = {min= 0, max = tmax_phy, interval = 100*dt},
    output = {format = 'vtk'}  --has to be added to the attributes right behind kind and label
  },
--  {
--    label = 'hvs',
--    folder = 'tracking/',
